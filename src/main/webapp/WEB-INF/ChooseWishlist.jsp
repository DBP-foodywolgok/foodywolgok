    ...
    <div class="container mt-4">
        <form id="wishlistForm" method="post" action="/restaurant/addWish">
            <div class="wishlist-container">
                ...
                <c:set var="restaurantDAO" value="<%= new model.dao.RestaurantDAO() %>" />
                <c:set var="wishlist" value="${requestScope.wishlist}" />
                <c:set var="restaurantId" value="${requestScope.restaurant_id}" />
                <c:if test="${not empty wishlist}">
                    <c:forEach var="item" items="${wishlist}">
                        <c:set var="isChecked" value="${restaurantDAO.containsRestaurant(item.getWishlist_id(), restaurantId)}" />
                        <div>
                ...
                <input type="hidden" name="restaurant_id" value="${requestScope.restaurant_id}" />
